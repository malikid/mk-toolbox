...
const ERROR_MESSAGE = {
  ...
  SOLD_PROPERTY:
    "There is no floor plan available for sold properties. Please provide an unsold property url!",
  NO_FLOOR_PLAN:
    "There is no floor plan found for this property. Please try another one!",
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const mediaViewerPlattegrondContainer = $(
      ".media-viewer-plattegrond-container",
      html
    );
    const fmlFileUrl = mediaViewerPlattegrondContainer.attr(
      "data-plattegrond-src"
    );

    const fmlPage = await browser.newPage();
    await fmlPage.goto(fmlFileUrl);
    const fmlHtml = await fmlPage.content();
    const webkitXmlViewerSourceXml = $(

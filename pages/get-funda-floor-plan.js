...
export default function GetFundaFloorPlan() {
  ...
  const submitUrl = async (event) => {
    ...
    const res = await fetch("/api/funda-fml", {
      body: JSON.stringify({
  ...
  return (
    ...
    >
      <Head>
        <title>MK Toolbox | Download Funda Floor Plan</title>
        <meta
          name="description"
          content="Get floor plans from funda by MK Toolbox"
        />
      ...
      <h3>
        <a href="https:\/\/floorplanner.com">Go to FloorPlanner</a> and import
        the floor plan!
      </h3>
      {isLoading && (
